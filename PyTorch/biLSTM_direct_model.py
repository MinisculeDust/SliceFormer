...
class BiLSTMImage(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.direction == 'horizontal':
            # Horizontal BiLSTM
            horizontal_input = x.permute(0, 2, 3, 1).contiguous().view(-1, 512, 3)
            horizontal_output, _ = self.horizontal_lstm(horizontal_input)
            horizontal_output = horizontal_output.view(-1, 256, 512,
            ...
            vertical_output = vertical_output.view(-1, 512, 256, 2 * lstm_hidden_size).permute(0, 2, 1, 3)
...
# model = BiLSTMImage(lstm_hidden_size)
